...
const getDaysInMonth = () => {
    ...
    let currentMonthDays = [];// creating an empty array for current month days
        //lastMonthDays = []; // creating an empty array for last month days
    for (let i = firstDayofMonth; i > 0; i--) { // loop through all days of last month
        currentMonthDays.push(new Day(`${lastDateofLastMonth - i + 1}`, 'inactive')); // push all days in last month days array
    }
    for (let i = 1; i <= lastDayofMonth; i++) {// loop through all days of current month
    ...
    for (let i = 1; i <= 6 - lastDayofNextMonth; i++) { // loop through all days of next month
...
icons.forEach(icon => { // getting prev and next icons
